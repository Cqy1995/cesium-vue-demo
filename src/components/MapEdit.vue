<template>
    ...
    <div style="position: absolute; top: 90%; left: 40%; padding: 0px; background-color: rgba(0, 0, 0, 0.2); border-radius: 10px;">
            <el-button-group>
    ...
    methods:{
        ...
        drawPoint:function(callback){
            ...
            handler.setInputAction(function (movement) {
                ...
                sessionStorage.setItem("point"+Math.random(),movement.position)
        ...
        drawLineString:function(callback){
            ...
            handler.setInputAction(function (movement) {
                ...
                test.y=y;
                test.z=z;
                sessionStorage.setItem("line"+ Math.random(),JSON.stringify(test))
            }, Cesium.ScreenSpaceEventType.RIGHT_CLICK);
        },
        //画面
        drawPolygon:function(callback){
            ...
            handler.setInputAction(function (movement) {
                ...
                test.y=y;
                test.z=z;
                sessionStorage.setItem("polygon"+Math.random(),JSON.stringify(test))
            }, Cesium.ScreenSpaceEventType.RIGHT_CLICK);
        },
        //画矩形
                ...
                if(tempPosition){
                    if(_self.shape.points.length==0) {
                        ...
                        for(let i=0;i<pointsArr.length;i++){
                            x.push(pointsArr[i].x)
                            y.push(pointsArr[i].y)
                            z.push(pointsArr[i].z)
                        }
                        test.x=x;
                        test.y=y;
                        test.z=z;
                        sessionStorage.setItem("rect"+Math.random(),JSON.stringify(test))
                    }
                }
            },Cesium.ScreenSpaceEventType.LEFT_CLICK);
            //鼠标移动
        ...
        clearHandle: function () {
            sessionStorage.clear();//清除sessionStorage里的数据
            //移除所有实体Entity
